    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-[#E8E4DE] bg-white p-6">
        ...
        <ul className="mt-3 space-y-1 text-sm text-[#172033]">
          ...
          <li className="text-[#a8aab0]">※ 환율 대략 {KRW_PER_USD.toLocaleString('ko-KR')}원/$ 기준</li>
        </ul>

        <div className="mt-4 rounded-xl border border-[#E8E4DE] bg-[#FAF8F4] p-4">
          <p className="text-sm font-bold text-[#172033]">📌 예시</p>
          <p className="mt-1 text-sm text-[#172033]">
            <b>30명</b> 기준, <b>1명당 3번씩</b> 로고송(15초)을 만들면 →{' '}
            <b>총 약 5,000원</b> 요금 발생
          </p>
          <p className="mt-1 text-xs text-[#e0562e]">
        ...
        <p className="mt-4 text-xs text-[#8a8f99]">
      ...
      <section className="rounded-2xl border border-[#E8E4DE] bg-white p-6">
        <div className="flex items-center gap-2">
          <h2 className="font-title text-lg font-bold text-[#172033]">📅 날짜별 요금 기록</h2>
          <button
            onClick={() => setReloadKey((k) => k + 1)}
            className="ml-auto rounded-full bg-[#F3EDE4] px-3 py-1.5 text-sm font-semibold text-[#172033] hover:bg-[#E8E4DE]"
          >
            🔄 새로고침
          </button>
        </div>
        ...
        {usage && (
          <div className="mt-3 flex flex-wrap gap-3 text-sm">
            <span className="rounded-lg bg-[#F3EDE4] px-3 py-1.5">
        ...
        <div className="mt-4 overflow-x-auto">
          {loading ? (
            <p className="py-8 text-center text-[#a8aab0]">불러오는 중…</p>
          ) : !usage || usage.days.length === 0 ? (
            <div className="rounded-xl border border-dashed border-[#E8E4DE] py-10 text-center text-sm text-[#8a8f99]">
            ...
            <table className="w-full min-w-[480px] text-sm">
              <thead>
                <tr className="border-b border-[#E8E4DE] text-left text-[#8a8f99]">
              ...
              <tbody>
                {usage.days.map((d) => (
                  <tr key={d.day} className="border-b border-[#F3EDE4]">
                    <td className="py-2 font-medium text-[#172033]">{d.day}</td>
                    ...
                    <td className="py-2 text-right font-semibold text-[#172033]">
                      {won(d.costKrw)}
                    </td>
                  </tr>
                ))}
